...
def upload():
    ...
    output_options = request.form.getlist('output_options')
    ...
    # Process the uploaded files based on selected options
    try:
        # Clipped DEM (meters)
        if 'clipped_dem' in output_options:
            clipped_dem_data, tmp_dir = clip_raster(dem_path, kml_path)
        
        # Clipped DEM (feet)
        if 'clipped_dem_feet' in output_options and 'clipped_dem' in output_options:
            clipped_dem_feet_data, tmp_dir = transform_to_feet(clipped_dem_data, tmp_dir, kml_file.filename)
        
        # Contours (Shapefile)
        if 'contours_shp' in output_options and 'clipped_dem_feet' in output_options:
            contour_shp_data, tmp_dir = generate_contours(clipped_dem_feet_data, tmp_dir, kml_file.filename)
        
        # Contours (DXF)
        if 'contours_dxf' in output_options and 'contours_shp' in output_options:
            dxf_data, tmp_dir = convert_shapefile_to_dxf(contour_shp_data, tmp_dir, kml_file.filename)
        
        # PVSyst Input (CSV)
        if 'pvsyst_csv' in output_options:
            points_data, csv_path, tmp_dir = raster_to_points(clipped_dem_data, tmp_dir, kml_file.filename)
        
        # 3D Points (DXF, meters)
        if 'points_dxf_meters' in output_options and 'pvsyst_csv' in output_options:
        ...
        # 3D Points (DXF, feet)
        if 'points_dxf_feet' in output_options and 'pvsyst_csv' in output_options:
        ...
        # Generated Mesh (DXF, feet)
        if 'mesh_dxf' in output_options and 'pvsyst_csv' in output_options:
        ...
        with zipfile.ZipFile(zip_buffer, 'w') as zipf:
            ...
            if 'clipped_dem_feet' in output_options:
            ...
            if 'contours_shp' in output_options:
            ...
            if 'contours_dxf' in output_options:
            ...
            if 'points_dxf_meters' in output_options:
            ...
            if 'points_dxf_feet' in output_options:
